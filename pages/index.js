      ...
      {users.slice(2, 3).map((user) => (
        <div key={user.id} className="flex flex-col items-center justify-center space-y-4">
          ...
          {user.fields['Full name'] &&
            <h1 className="text-2xl font-bold font-open-sans text-center mt-4 mb-8 tracking-wide" style={{fontSize: '20px', color: '#F8F2EA' }}>
              {user.fields['Full name']}
            </h1>}
          {user.fields['Position and company name'] &&
            <p className="text-1xl leading-6 tracking-wider" style={{fontSize: '15px', marginBottom: '50px', color: '#F8F2EA' }}>{user.fields['Position and company name']}</p>}
          ...
          {user.fields.Website &&
            <button style={{ backgroundColor: '#F8F2EA', borderRadius: '10px', padding: '15px 120px', fontSize: '15px', marginBottom: '20px', color: '#656539' }} onClick={() => window.open(user.fields.Website, '_blank')}><strong>Website</strong></button>}
          {user.fields.LinkedIn &&
            <button style={{ backgroundColor: '#F8F2EA', borderRadius: '10px', padding: '15px 117px', fontSize: '15px', marginBottom: '20px', color: '#656539' }} onClick={() => window.open(user.fields.LinkedIn, '_blank')}><strong>LinkedIn</strong></button>}
